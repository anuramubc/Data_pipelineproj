...
def build_insert_query(table_name, column_names):
    ...
    query = (f'INSERT INTO {table_name} ({column_names_string}) VALUES ({column_values_string})')
    return query
...
def load_table(db_details, data, column_names, table_name):
    ...
    connection = get_connection(db_type = TARGET_DB['DB_TYPE'],db_host = TARGET_DB['DB_HOST'], db_name= TARGET_DB['DB_NAME'],
                                       db_user = TARGET_DB['DB_USER'], db_pass =TARGET_DB['DB_PASS'])
